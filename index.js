// index.js
// === Servicio24: emojis oficiales (NO CAMBIAR) ===
// Servicios:
...
function sendCityMenu(to) {
  return axios.post(GRAPH_URL,{
    ...
    interactive:{
      ...
      footer:{ text:"Servicio24" },
      action:{ button:"Elegir ciudad", sections:[
...
function sendZonaList(to,start,end){
  const rows=[];
  for(let z=start;z<=end;z++){ rows.push({ id:`zona_${z}`, title:`${ZONA_EMOJI[z]} Zona ${z}` }); }
  return axios.post(GRAPH_URL,{
    messaging_product:"whatsapp",to,type:"interactive",
    interactive:{
      type:"list",
...
function sendZonaConfirm(to,z){
  ...
  return axios.post(GRAPH_URL,{
    ...
    interactive:{
      ...
      footer:{ text:"Servicio24 • Al continuar aceptas recibir contactos sin costo" },
      action:{ buttons:[
...
function sendServicesList(to,cityTitle,z){
  ...
  return axios.post(GRAPH_URL,{
    ...
    interactive:{
      ...
      footer:{ text:"Servicio24 • Al continuar aceptas recibir contactos sin costo" },
      action:{ button:"Elegir servicio", sections:[{ title:"Profesionales", rows:[
...
function sendLeadReady(to,cityTitle,z,serviceId){
  const service=SERVICE_LABEL[serviceId]||"Profesional";
  const emoji=ZONA_EMOJI[z]||"";
  const text=`Listo ✅ ${service} • Zona ${z} ${emoji} • ${cityTitle}.\nEn breve proveedores se contactarán contigo.\n\nServicio24`;
      ...
      for(const msg of messages){
        ...
        if(msg.type==="text"){
          ...
          if(!s.lastWelcome||now-s.lastWelcome>SESSION_TTL_MS){
            await sendRoleButtons(from); s.lastWelcome=now;
          }
          continue;
        }
        ...
        if(interactive?.type==="list_reply"){
          ...
          if(CITIES.some(c=>c.id===id)){ s.city=CITIES.find(c=>c.id===id); await sendZonaGroupButtons(from); continue; }
          if(id?.startsWith("zona_")){ const z=parseInt(id.split("_")[1],10); if(z>=1&&z<=25){ s.zone=z; s.zoneConfirmed=false; await sendZonaConfirm(from,z); continue; }}
          if(SERVICE_LABEL[id]){ s.serviceId=id; if(s.zoneConfirmed){ const cityTitle=s.city?.title||"Ciudad de Guatemala"; await sendLeadReady(from,cityTitle,s.zone,s.serviceId);} else { await sendZonaGroupButtons(from);} continue;}
        }
        if(interactive?.type==="button_reply"){
          ...
          if(id==="zona_confirm"){ if(!s.zone){ await sendZonaGroupButtons(from); continue;} s.zoneConfirmed=true; const cityTitle=s.city?.title||"Ciudad de Guatemala"; await sendServicesList(from,cityTitle,s.zone); continue;}
        }
        if(!s.city){ await sendCityMenu(from); continue;}
      }
    }
    return res.sendStatus(200);
  }catch(err){ console.error("Webhook POST error:",err?.response?.data||err.message); return res.sendStatus(200);}
});
